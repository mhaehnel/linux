...
//[*]--------------------------------------------------------------------------------------------------[*]
#ifdef CONFIG_PM
static int 	ina231_i2c_suspend(struct device *client, pm_message_t message)
{
	#ifdef CONFIG_HAS_EARLYSUSPEND
		struct	ina231	*sensor = i2c_get_clientdata(to_i2c_client(client));
	
		sensor->pdata->suspend(&to_i2c_client(client)->dev);
	#endif
...
//[*]--------------------------------------------------------------------------------------------------[*]
static int 	ina231_i2c_resume(struct device *client)
{
	#ifdef CONFIG_HAS_EARLYSUSPEND
		struct	ina231	*sensor = i2c_get_clientdata(to_i2c_client(client));

		sensor->pdata->resume(&to_i2c_client(cliet)->dev);
	#endif	
...
static 	void 	ina231_work		(struct work_struct *work)
{
	...
	sensor->cur_uJ += (sensor->cur_uW / 1000 ) * (sensor->pd->update_period / 1000);
        ...
        sensor->cur_uV = 0; sensor->cur_uA = 0; sensor->cur_uW = 0; sensor->cur_uJ = 0;
    }
